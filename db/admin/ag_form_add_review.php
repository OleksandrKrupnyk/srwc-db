<header><a href="action.php?action=view#id_w<?= $_GET['id_w']?>">Усі роботи</a></header>
...
">
    ...
    <fieldset name="descriptionWorks" id="descriptionWorks">
        ...
        <p>
            ...
            $strArray = array();
            if ($row['introduction'] <> "") {
                array_push($strArray, "<strong>Впровадженння:</strong>{$row['introduction']}");
            }
            if ($row['public'] <> "") {
                array_push($strArray, "<strong>Результати опубліковано:</strong>{$row['public']}");
            }
            if ($row['comments'] <> "") {
                array_push($strArray, "<strong>Коментар/зауваження до матеріалів:</strong>{$row['comments']}");
            }
            if (count($strArray) < 1) {
...
<table>
    ...
    <input type="submit" value="Записати"><input type="button" value="Повернутися" name="return"
                                                 onclick="window.location='action.php?action=view#id_w'+<?= $_GET['id_w'] ?>">
